...
export const pages: GrammarPage[] = [
  {
    ...
    brPt:
      "Орфографія 16–19 і `cinquenta`/`cinqüenta` (після угоди — `cinquenta` скрізь): `dezasseis` (PT-PT) vs `dezesseis` (PT-BR), `dezanove` vs `dezenove`. Великі числа: PT-PT `mil milhões` / `bilião` (довга шкала в традиційній нормі); PT-BR `bilhão` / `trilhão` (коротка шкала в звичайному вжитку). `Um mil` частотніше в PT-BR; PT-PT віддає перевагу голому `mil`.",
    regional:
  ...
  },
  {
    ...
    formulas: [
      {
        ...
        note: "Питання завжди у множині: Que horas são? не Que hora é? (окрім «яка саме година з розкладу»).",
      },
      {
        label: "Дата",
    ...
    mistakes: [
      mistake(
        "É duas horas.",
        "São duas horas.",
        "Від двох годин дієслово в множині. Однина лише для `uma hora`, `meio-dia`, `meia-noite`.",
      ),
      mistake(
    ...
    brPt:
      "PT-BR: `quinze para as três`, `três e quinze`, часто `da manhã/tarde`. PT-PT: `três menos quinze`, `um quarto para as três`, `três e um quarto`. Слова `comboio` (PT) / `trem` (BR) у прикладах дат не граматика, а лексика. Обидва вживають `às 8h`.",
    regional:
      "В офіційному мовленні АН і MZ дати й 24-годинний формат ближчі до європейської канцелярії. Усне «за п'ятнадцять» варіює між моделями `para as` і `menos`.",
    tables: [
      {
        caption: "Годинник",
