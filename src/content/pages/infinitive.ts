...
export const pages: GrammarPage[] = [
  {
    ...
    formation:
      "Безособовий інфінітив — це форма на `-ar`, `-er`, `-ir` (і `pôr` з похідними). Наголос на тематичному голосному: `fa**lar**`, `co**mer**`, `par**tir**`.\n\nНемає особових закінчень. Форма одна для всіх осіб: `falar` може відповідати «говорити мені / тобі / їй / нам», якщо контекст уже задав підмет.\n\nКлитики в PT-PT часто енклітичні: `quero vê-lo`. У розмовному PT-BR займенник часто стоїть після інфінітива: `quero ver ele`; нормативні письмові варіанти залежать від конструкції та регістру. Не зводьте це до правила «PT-BR завжди postverbal».",
    uses: [
      {
        title: "Після модальних і фазових дієслів",
        body: "`poder`, `dever`, `querer`, `saber` («уміти»), `conseguir`, `precisar` (PT-BR часто без прийменника), `ir`, `voltar a`, `começar a`, `deixar de`, `acabar de` у звичайних перифрастичних моделях поєднуються з нефлексованим інфінітивом. Підмет зазвичай уже виражений у головній клаузі. Окремі інфінітивні конструкції можуть мати власний суб'єкт і тоді допускають/вимагають іншу форму.",
        examples: [
    ...
    exceptions:
      "Безособовий інфінітив часто замінюють особовим, коли треба однозначно показати окремого виконавця інфінітивної дії і його не можна надійно відновити з контексту. Але це не механічне правило: вибір між особовим і безособовим інфінітивом залежить також від кореферентності, структури та фокусу. Тоді потрібен особовий інфінітив або повне речення з кон'юнктивом: коли потрібно явно виразити окремого виконавця: `para chegarem cedo` / `para que cheguem cedo`; не робіть із `para chegar cedo` автоматично «помилку», якщо виконавець відновлюється з контексту.\n\nУ європейському прогресиві `estar a + infinitivo` зазвичай має той самий суб'єкт, що й `estar`: `estou a trabalhar`. Це звичайна перифрастична конструкція, а не загальна заборона на особовий інфінітив після прийменника `a` в усіх інших контекстах.",
    mistakes: [
      mistake(
