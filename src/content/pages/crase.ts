...
export const pages: GrammarPage[] = [
  {
    ...
    titleEn: "Crase",
    summary:
      "Злиття прийменника a з артиклем a (або з початковим a- вказівного aquele). Орфографія обох стандартів, не «бразильська вигадка».",
    ...
    intro:
      "Crase — це **злиття двох a**: прийменника `a` і означеного артикля `a` (або `as`, або початкового `a` у `aquele/aquela/aquilo`). На письмі злиття позначають гравісом: `à`, `às`, `àquele`, `àquela`, `àquilo`. Вимова в багатьох акцентах збігається з простим `a` — тому явище передусім **орфографічне**.",
    ...
    formation:
      "Гравіс (`acento grave`) у сучасній португальській майже не використовується ні для чого, крім crase. Це не наголосовий знак на кшталт акута в `já`: він **сигналізує злиття**.",
    ...
    exceptions:
      "Crase **неможлива** перед чоловічим іменником, перед більшістю дієслів, перед `um/uma`, перед голими топонімами без артикля, перед `ela/você/quem` у типових рамках. Детальний розподіл — обов'язкова / факультативна / заборонена.",
    ...
    ukrainian:
      "Українська не має ні артикля, ні злиття `до+та`. `Йду до школи` = `vou à escola` лише як **цілий еквівалент**, не по частинах. Друге `a` українець не «чує», бо його рідна граматика не вимагає означеного артикля перед `escola`.",
    ...
    brPt:
      "Орфографія `à/às/àquele` спільна. PT-PT частіше пояснює явище як рядок контракцій `ao/à`. PT-BR має величезну шкільну літературу «crase obrigatória / facultativa / proibida». Зміст збігається більше, ніж тон підручників.",
    ...
    regional:
      "Африканські офіційні тексти використовують `à` за європейською орфографією. Немає окремої «ангольської crase». Помилки типу пропуску гравіса всюди однакові за механізмом: не побачили артикля.",
    tables: [
      {
  ...
  },
  {
    ...
    summary:
      "Жіночий визначений іменник після прийменника a, годинник às, locuções, àquele. Там, де чоловічий ряд дає ao.",
    aliases: ["à", "обов'язкова crase", "crase obrigatória", "às horas"],
    related: ["crase", "crase-optional", "a-vs-crase", "a-article", "prep-a"],
    intro:
      "Обов'язкова crase — не список «слів із à», а наслідок двох умов: прийменник `a` **і** жіночий означений артикль (або `aquele…`). Коли обидві умови стабільні, гравіс не стоїть на розсуд автора.",
    ...
    formation:
      "Не додавайте гравіс до будь-якого `a` перед жіночим словом. Потрібен саме прийменник. У `a casa é antiga` перше `a` — артикль підмета, crase немає. У `fui à casa da Ana` прийменник + артикль — crase є.",
    uses: [
      {
    ...
    exceptions:
      "Навіть «обов'язкові» зони скасовуються, якщо артикль зникає: `ir a Lisboa` (місто без артикля), `assistir a espetáculos` (гола множина / нуль у деяких уживаннях), `chegar a casa` (ідіома).",
    ...
    ukrainian:
      "Список «завжди ставте à перед школою, пляжем, годиною» працює як милиця, але ламається на `Lisboa`, `Portugal`, `ela`, `trabalhar`. Тримати треба умови (прийменник + артикль), а милиці — як частотні ілюстрації.",
    brPt:
      "`assistir a` vs `assistir` (прямий додаток) змінює наявність crase: BR `assisti o jogo` (розмовно/поширено) vs PT `assisti ao jogo`. Перед `à peça` в обох, якщо регенція з `a` збережена.",
    tables: [
      {
  ...
  },
  {
    ...
    summary:
      "Перед жіночими іменами та присвійними гравіс часто на вибір; перед чоловічим, дієсловом, um, ela — його бути не може.",
    aliases: [
      "факультативна crase",
      "crase proibida",
      "заборонена crase",
    ...
    intro:
      "Шкільна тріада Бразилії — obrigatória / facultativa / proibida — зручна, якщо не перетворювати її на магію. Факультативність з'являється там, де **артиклю можна не бути**: жіноче особове ім'я, присвійний (особливо з огляду на PT-BR), деякі `até a`. Заборона — там, де другого `a` немає в принципі.",
    ...
    exceptions:
      "`até` + жіночий означений: шкільні джерела розходяться (`até às 15h` vs `até as 15h`). Обидва трапляються; `até às` читає `até` + прийменник `a` + артикль, `até as` — `até` безпосередньо + артикль. Виберіть одну логіку в тексті.",
    ...
    ukrainian:
      "Заборонені зони збігаються з тим, де українець і так не шукає «означеності»: інфінітив, `вона`, `один`. Пастка — навпаки: після уроків про crase почати ставити `à` перед кожним жіночим словом. Фільтр: чи є прийменник `a`? чи є саме означений артикль `a`?",
    brPt:
      "Факультативність перед іменами й присвійними — майже прямий відбиток регіональної дистрибуції артикля. PT-PT зсуває ці зони до «практично обов'язково»; PT-BR — до «часто без». Шкільна етикетка facultativa корисна саме в бразильському контексті, де обидві орфографії живуть поруч.",
    regional:
      "Не оголошуйте факультативність універсальною нормою Африки чи Португалії. Описуйте її як наслідок опціональності артикля.",
    tables: [
      {
        caption: "Три кошики",
  ...
  },
  {
    ...
    titleEn: "a vs à",
    summary:
      "Тест заміни на чоловічий іменник: якщо з'являється ao, жіночому потрібне à. Найнадійніший спосіб не вгадувати гравіс.",
    aliases: [
      "a чи à",
      "як перевірити crase",
      "ao teste",
    ...
    intro:
      "Розрізнити `a` і `à` вухом важко. Надійний письмовий тест: **замініть жіночий іменник чоловічим** (з тим самим прийменником і тією самою визначеністю). Якщо в чоловічому варіанті бачите `ao`, у жіночому пишіть `à`. Якщо в чоловічому лишається голе `a`, гравіса немає.",
    ...
    formation:
      "Алгоритм: (1) Чи є прийменник `a`? Якщо група — підмет або прямий додаток, стоп: артикль `a` без гравіса. (2) Чи є означений артикль жіночий / `aquele`? Якщо нуль, `um/uma`, займенник — стоп. (3) Зробіть чоловічу заміну. (4) Зафіксуйте орфографію.",
    ...
    exceptions:
      "Тест слабшає, коли чоловічого іменника з тим самим значенням немає (`à praia` — `ao mar` близьке, але не синонім). Тоді беріть будь-який чоловічий іменник **тієї самої синтаксичної рамки**: `vou à praia` / `vou ao clube`.",
    ...
    ukrainian:
      "Українська не допоможе почути різницю `a/à`. Допоможе **свідома підстановка**, якої в рідній мові немає. Зробіть тест звичкою на письмі, як перевірку роду: не інтуїція «чи жіноче», а «чи є ao в мінімальній парі».",
    ...
    brPt:
      "Тест спільний і в Ріо, і в Лісабоні. Відрізняється вхід: чи стоїть артикль перед `Maria` / `minha casa`. Сам механізм `ao ⇒ à` не регіональний. Не кажіть, що європейці «не роблять crase»: вони пишуть `à` так само, просто рідше влаштовують із цього окремий предмет.",
    regional:
      "В іспитах Бразилії (vestibular, concursos) цей тест — стандартний прийом. Він так само валідний для редагування європейського тексту. Користуйтеся ним як інструментом, не як національною емблемою.",
    tables: [
      {
