...
export const pages: GrammarPage[] = [
  {
    ...
    summary: "Обов'язкове злиття прийменника з артиклем: do, no, ao, pelo. Це морфонологічна парадигма, не список слів.",
    aliases: [
      "контракції",
      "contrações",
      "do",
    ...
    ukrainian: "Українські прийменники `з`, `у`, `до`, `через` не зливаються з вказівними. Є лише орфографічні речі на кшталт `з+з` → `із/зі` за милозвучністю — це не та сама операція. Португальська контракція **граматично обов'язкова** й змінює вигляд слова: `de` зникає як окремий графічний елемент.\n\nНе перекладайте `do` завжди як «від / з / того». `do` = `de` + `o`, а значення дає сам `de` (належність, джерело, тема…). Український родовий (`книжка викладача`) часто відповідає `de` + артикль, але не кожен родовий = `do`.",
    brPt: "Таблиця `do/no/ao/pelo` спільна й обов'язкова в обох стандартах. Різниця знову в тому, **чи є артикль**: PT-PT `da Maria`, PT-BR часто `de Maria`. Сама форма `da` не «європейська».\n\nРозмовні `pro/pra` — сильна прикмета PT-BR. У PT-PT розмовно є `pá` (інша етимологія, вигук) і скорочення, але письмова парадигма та сама.",
    regional: "В африканських різновидах писемна норма зберігає повну таблицю контракцій. Усне мовлення може розділяти або редукувати форми; для довідника орієнтир — стандартна орфографія.",
    tables: [
      {
  ...
  },
  {
    ...
    intro: "`de` + `o/a/os/as` дає `do/da/dos/das`. Це найчастотніший ряд: належність, джерело, матеріал, тема, частина `gostar de`, інфінітивні рамки `antes de`, `depois de`.\n\nНе вчіть `do` як окремий прийменник «від». Спочатку бачте `de`, потім перевіряйте, чи наступне слово — означений артикль.",
    formulas: [
      {
        label: "Ряд de",
    ...
    ukrainian: "Український родовий (`книжка брата`, `столиця України`) часто відповідає `de` + група. Але родовий також покриває частину відмінків, де португальська візьме інший прийменник (`про проблему` = `do assunto` / `sobre o assunto`). Не ставте `do` під кожен український родовий.",
    brPt: "Форми спільні. `da Maria` vs `de Maria` — наслідок артикля при імені, не наслідок іншої контракції. `preciso do livro` vs BR розмовне `preciso o livro` (без de) — уже керування, не таблиця do/da.",
    tables: [
      {
  ...
  },
  {
    ...
    formation: "`em` + `o` історично дало `no` (n- з носового прийменника). Орфографія сучасна: завжди `no`, ніколи `em o`. Множина `nos` збігається графічно з займенником `nos` («нам/нас») — омографія: `Viram-nos no parque` (займенник + контракція) розрізняється синтаксисом.\n\n`em um` → часто `num` / `numa` в мовленні й у PT-PT письмі; `em um` теж коректне, особливо в формальному PT-BR.",
    uses: [
      {
        title: "Місце",
    ...
    ukrainian: "`у/в` + місцевий відмінок (`у школі`, `в Бразилії`) покриває і `no/na`, і голе `em`. Португальська різниця видима лише через наявність артикля. Навчіться питати «чи є o/a?», а не «як перекладається в».",
    brPt: "Парадигма спільна. Лексика вмістилищ різна (`comboio` PT / `trem` BR, `frigorífico` PT / `geladeira` BR), контракція та сама: `no trem`, `no comboio`. `num/numa` частотніші в європейському письмі, ніж у формальному бразильському, але зрозумілі скрізь.",
    tables: [
      {
  ...
  },
  {
    ...
    titlePt: "a + artigo",
    titleEn: "a + article",
    summary: "ao, à, aos, às: напрямок, час, непрямий додаток. Жіноче à — це crase (a + a).",
    ...
    intro: "`a` + означений артикль → `ao` (чоловічий), `à` (жіночий), `aos`, `às`. Чоловічий ряд ніхто не називає crase; жіночий `à` — це і контракція, і орфографічна crase (гравіс).\n\nДля україномовного важливо не розвести `ao` і `à` по різних полицях. Це **один ряд**. Різниця лише в роді артикля. Тест заміни на чоловічий іменник (якщо з'являється `ao`, жіночому потрібне `à`) працює саме тому, що ряд симетричний.\n\nПрийменник `a` покриває напрямок (`vou ao médico`), час (`às três`, `aos domingos`), адресата (`dei o livro à Ana` / `ao João`), низку керування (`assistir a`, `obedecer a`).",
    formulas: [
      {
        label: "Ряд a",
        pattern: "a + o/a/os/as → ao/à/aos/às",
        note: "à = a (прийменник) + a (артикля). На письмі гравіс. Вимова збігається з a в багатьох акцентах — звідси трудність.",
      },
    ],
    formation: "`a` + `o` → `ao` (два голосні, без гравіса). `a` + `a` → `à` (гравіс позначає злиття). Множина: `a` + `os` → `aos`; `a` + `as` → `às`.\n\nБез артикля лишається голе `a`: `a Portugal` (рідко потрібне), `a pé`, `a Maria` в PT-BR як додаток без прийменника — інший `a` (артикль) або його відсутність. Не плутати три `a`: артикль, прийменник, клитик-займенник.",
    uses: [
      {
    ...
    ukrainian: "Українські `до`, `на`, `о` (о третій), давальний адресата (`директору`) розмазані по кількох португальських прийменниках. `ao/à` — лише ті випадки, де вибрано саме `a` **і** є означений артикль. Давальний `їй` = `a ela` / `lhe`, не `à ela`.",
    brPt: "Ряд спільний. Школи в Бразилії окремо натаскуюють crase (`à`); у Португалії `à` часто пояснюють просто як контракцію `a+a`, без культу теми. Орфографія однакова. `ir à praia` / `ir ao médico` нормальні в обох різновидах.",
    tables: [
      {
  ...
  },
  {
    ...
    titlePt: "por + artigo",
    titleEn: "por + article",
    summary: "pelo, pela, pelos, pelas: агенс, причина, маршрут, тривалість. Не плутати з pelo «волосся» (старе pêlo).",
    ...
    intro: "`por` + означений артикль → `pelo/pela/pelos/pelas`. Історично `per` + `lo`. Сучасний мовець не збирає форму на ходу як `por+o`, але граматично ряд саме такий і **обов'язковий**: не пишуть `por o`.\n\nЗначення дає `por`: агенс пасиву (`assinado pelo diretor`), причина, плата, маршрут (`pelo parque`), тривалість (`pelo inverno` в низці контекстів), приблизна оцінка (`pelas duas horas`).",
    formulas: [
      {
        label: "Ряд por",
    ...
    examples: [
      ...
      ex("Pela manhã estou mais lúcido.", "Вранці я ясніший / більш ясний."),
    ...
    ukrainian: "Українські `через`, `по`, `за` (за книжку заплатив), орудний агенса (`підписано директором`) можуть відповідати `por` і далі `pelo`, якщо є артикль. Але `до лікаря` — не `pelo`. Вибір прийменника первинний; контракція вторинна.",
    brPt: "Ряд спільний. `pela manhã` живе в обох; PT-BR також дуже любить `de manhã` (нуль). Обидва варіанти нормальні, різниця частотна.",
    tables: [
      {
  ...
  },
  {
    ...
    summary: "neste, deste, àquele, daquilo: ті самі прийменники de / em / a, злиті з este / esse / aquele і isto / isso / aquilo.",
    aliases: [
      "neste",
      "deste",
      "àquele",
    ...
    intro: "Вказівні `este/esse/aquele` і нейтральні `isto/isso/aquilo` теж зливаються з `de`, `em`, `a`. Це продовження тієї самої системи, не нові прийменники. `neste` = `em` + `este`; `deste` = `de` + `este`; `àquele` = `a` + `aquele` (і знову гравіс, бо a+a).\n\nУкраїнські `цей/той` після прийменника лишаються окремими словами (`у цьому`, `з того`). Португальська зсуває межу слова.",
    formulas: [
      {
        label: "Модель",
    ...
    ukrainian: "`у цьому`, `з того`, `до того` — три слова. Португальська пакує прийменник у ту саму словоформу, що й вказівний. Для читання розбирайте `neste` назад на `em`+`este`, інакше здаватиметься, що є десятки прийменників.",
    brPt: "Форми спільні. Різниця — у виборі `este/esse` (PT-BR у мовленні частіше `esse` там, де PT-PT поставить `este`). Контракція тоді буде `nesse` замість `neste`, але правило злиття те саме.",
    tables: [
      {
  ...
  },
  {
    ...
    summary: "dele/dela, nele/nela, consigo: злиття з ele/ela та особливі форми з com. Відрізняйте dele від seu.",
    ...
    ukrainian: "`його книжка` = `o livro dele` або `o seu livro`. Українське `його` однозначніше за `seu`, тому `dele` часто рятує. `у ньому` = `nele`. `зі мною` = `comigo`, не калька «з + мене».\n\nНе ставте артикль перед `ele`: *`o dele` як «його» можливе лише субстантивовано (`o dele está ali` = «його (предмет) там»), на зразок українського «його лежить там».",
    brPt: "`dele/nele` спільні. Різниця `connosco` (PT-PT) vs `conosco` (PT-BR) — орфографія після угоди: Бразилія пише одне `n`, Португалія зберігає `nn` у цій формі (як `conexão` vs старіші звички — тут саме стійка відмінність connosco/conosco).\n\nPT-BR частіше розв'язує `seu` через `dele/dela` в мовленні, бо `seu` ще й «ваш (você)». У PT-PT `seu` часто «його/її», а «ваш» — `o seu` до senhor або `o teu` до tu.",
    tables: [
      {
