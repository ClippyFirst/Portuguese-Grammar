...
## Batch 40 — structural repair + learner-facing accuracy (2026-09-26)

Продовжено роботу за snapshot Batch 40, але актуальним джерелом істини взято поточний `main`. Старі TS/syntax findings із snapshot уже були усунені послідовними structural commits; повторно їх не відтворювали.

### Виправлено

- Відновлено валідну структуру компактних `p(...)` сторінок після snapshot-помилок у `clitics.ts`, `crase.ts`, `moods.ts`, `se.ts` та пов'язаних модулях; parser content audit також було посилено для вкладених дужок і балансування конструкторів.
- `verbs.ts`: виправлено український глос парадигми `sou, és, é`, щоб він не містив хибного «єш» для `és`.
- `prepositions.ts`: виправлено помилковий запис `vosco` на нормативне `convosco` у PT-PT контексті.
- `se.ts`: уточнено опис складених часових форм із `se` та конструкції `trata-se de`, щоб навчальний текст не перетворював частотність або синтаксичну модель на необґрунтоване «ніколи/не існує».

### QA

- Перевірено змінені prose-фрагменти через Academic Writing Toolkit: для першого короткого ізольованого блоку отримано лише технічний `short-paragraph` сигнал; для другого — 0 issues. Це не є змістовою логічною помилкою.
- Повний локальний `npm` прогін у цьому середовищі не виконано: пряме клонування GitHub через shell заблоковане DNS. Тому `typecheck/lint/test/build` green не заявляється без фактичного виконання.
- Поточний `main` залишається джерелом істини; snapshot-файли не використовуються як список автоматичних повторних виправлень.
